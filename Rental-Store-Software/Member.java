...
public class Member 
{
    ...
    //private List<Transaction> cd_issued_details = new ArrayList<>(); //<transaction id,customer name,cd name,date>
    private int video_cds_issued,music_cds_issued;
    //private int 
    ...
    public String issueItem(Item cd)
    {
        ...
        if( (video_cds_issued<2 && cd.getItemFormat().equals("music")==false) || (music_cds_issued<1 && cd.getItemFormat().equals("music")==true) || isPremium==true) 
        {
            Transaction tr=new Transaction(name,cd_name);
            //store this transaction in database table
            //store in pl acc
            if(cd.getItemFormat().equals("music")==true) music_cds_issued+=1;
            if(cd.getItemFormat().equals("music")==false) video_cds_issued+=1;
